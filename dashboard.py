"""
YUNA Dashboard - Live monitoring with boarding pass and timeline
"""
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .boarding-pass .city-time { font-size: 1.1em; color: #e74c3c; margin-top: 5px; }
...
<body>
    <div class="container">
        <h1>🫶 Yuna</h1>
        {% if state.travel_state == 'mochi_time' %}
        ...
        <div class="grid">
            <div class="card">
                ...
                <div class="status-grid">
                    <div class="status-item"><div class="label">Location</div><div class="value">{{ state.location }}</div></div>
                    <div class="status-item"><div class="label">Day</div><div class="value">Day {{ state.days_in_location }}</div></div>
                    <div class="status-item"><div class="label">Local Time</div><div class="value">{{ state.local_time }}</div></div>
                    <div class="status-item"><div class="label">Status</div><div class="value">{{ state.travel_state or 'idle' }}</div></div>
                    <div class="status-item"><div class="label">Mood</div><div class="value">{{ state.mood }}</div></div>
                    <div class="status-item"><div class="label">Jet Lag</div><div class="value">{{ "Day " ~ state.jet_lag_days if state.jet_lag_days > 0 else "None" }}</div></div>
                </div>
                <div class="status-item" style="margin-top: 12px;"><div class="label">Energy</div><div class="value">{{ state.energy }}%</div><div class="energy-bar"><div class="energy-fill" style="width: {{ state.energy }}%"></div></div></div>
            ...
            <div class="card">
                ...
                {% if latest_thought %}<div class="thought-box">"{{ latest_thought.entry }}"</div><div class="post-meta">{{ latest_thought.location }} · {{ latest_thought.time_ago }}</div>{% else %}<p style="color: #666;">No thoughts yet...</p>{% endif %}
            </div>
            {% if travel.flight %}
            <div class="card travel-card">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                    <div>
                        <div class="boarding-pass">
                            ...
                            <div class="route">
                                <div class="city"><div class="city-code">{{ travel.flight.from_city[:3] | upper }}</div><div class="city-name">{{ travel.flight.from_city }}</div><div class="city-time">{{ travel.flight.dep }}</div></div>
                                <div class="plane-icon">✈️</div>
                                <div class="city"><div class="city-code">{{ travel.flight.arr_airport[:3] | upper }}</div><div class="city-name">{{ travel.flight.to_city }}</div><div class="city-time">{{ travel.flight.arr }}</div></div>
                            </div>
                            <div class="details">
                        ...
                        {% if travel.hotel %}<div class="hotel-card"><div class="hotel-name">🏨 {{ travel.hotel.name }}</div><div class="hotel-area">{{ travel.hotel.area }}, {{ travel.hotel.city }}</div><div class="hotel-price">${{ travel.hotel.price_per_night }}/night</div></div>{% endif %}
                    </div>
                    <div>
                        <h3 style="color: #888; margin-bottom: 10px;">📅 Timeline</h3>
                        <div class="timeline">
                            {% for step in travel.timeline %}
                            <div class="timeline-item {% if loop.index <= travel.current_step_index + 1 %}completed{% endif %} {% if loop.index == travel.current_step_index + 1 %}current{% endif %}">
                ...
                {% if travel.current_trip and travel.current_trip.reason %}<div style="margin-top: 15px; padding: 10px; background: rgba(255,255,255,0.05); border-radius: 8px; color: #aaa;">📝 {{ travel.current_trip.reason }}</div>{% endif %}
            </div>
            {% endif %}
            <div class="card">
                <h2>📸 Recent Posts</h2>
                {% if posts %}{% for post in posts[:5] %}
                <div class="post-item">
                    {% if post.content_url %}<div class="post-media"><img src="{{ post.content_url }}" onerror="this.style.display='none'"></div>{% endif %}
                    {% if post.caption %}<div class="post-caption">"{{ post.caption }}"</div>{% endif %}
                    <div class="post-meta">{{ post.location }} · {{ post.time_ago }}</div>
                </div>
                {% endfor %}{% else %}<p style="color: #666;">No posts yet...</p>{% endif %}
            </div>
            <div class="card">
                <h2>📓 Journal</h2>
                {% if journal_entries %}{% for entry in journal_entries[:5] %}
                <div class="journal-entry">{{ entry.entry[:150] }}{% if entry.entry|length > 150 %}...{% endif %}<div class="journal-meta">{{ entry.mood }} · {{ entry.location }} · {{ entry.time_ago }}</div></div>
                {% endfor %}{% else %}<p style="color: #666;">No entries yet...</p>{% endif %}
            </div>
            <div class="card">
                <h2>🗺️ Journey</h2>
                {% if journey %}{% for stop in journey[:8] %}
                <div class="journey-item"><strong>{{ stop.location }}</strong>{% if stop.notes %}<div style="font-size: 0.85em; color: #aaa;">{{ stop.notes }}</div>{% endif %}<div class="journal-meta">{{ stop.time_ago }}</div></div>
                {% endfor %}{% else %}<p style="color: #666;">Journey starting...</p>{% endif %}
            </div>
        </div>
...
def get_state():
    ...
    return {"location": state.get("location", "Singapore"), "energy": state.get("energy", 70), "mood": state.get("mood", "peaceful"), "jet_lag_days": state.get("jet_lag_days", 0), "days_in_location": state.get("days_in_location", 1), "local_time": local_time.strftime("%I:%M %p"), "home_base": home.get("name", "") if home else "", "travel_state": state.get("travel_state", ""), "mochi_hugs_today": state.get("mochi_hugs_today", 0)}
...
def dashboard():
    ...
    return render_template_string(DASHBOARD_HTML, state=state, travel=travel, posts=posts, journal_entries=journal, journey=journey, latest_thought=latest_thought, now=datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
...
if __name__ == "__main__":
    print("🫶 Yuna Dashboard: http://152.42.177.43:5000")
